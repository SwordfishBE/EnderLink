...
public final class ModrinthUpdateChecker {
    private static final String PROJECT_ID = "1EdUpHRe";
    ...
    public static void checkOnceAsync() {
        ...
        Thread thread = new Thread(ModrinthUpdateChecker::checkForUpdate, "enderlink-modrinth-update-check");
    ...
    private static void checkForUpdate() {
        HttpRequest request = HttpRequest.newBuilder()
            ...
            .header("User-Agent", EnderLinkMod.modName() + "/" + currentVersion())
            .GET()
            .build();
        ...
        try {
            ...
            if (response.statusCode() < 200 || response.statusCode() >= 300) {
                EnderLinkMod.LOGGER.debug("{} Update check returned HTTP {}.", EnderLinkMod.logPrefix(), response.statusCode());
                return;
            }

            Optional<String> latestVersion = extractLatestVersion(response.body());
            if (latestVersion.isEmpty()) {
                EnderLinkMod.LOGGER.debug("{} Update check returned no usable versions.", EnderLinkMod.logPrefix());
                return;
            }
            ...
            if (isNewerVersion(newestVersion, currentVersion)) {
                EnderLinkMod.LOGGER.debug("{} New version available: {} (current: {})",
            ...
            EnderLinkMod.LOGGER.debug("{} Update check failed.", EnderLinkMod.logPrefix(), exception);
        }
    }
    ...
    private static Optional<String> extractLatestVersion(String responseBody) {
        ...
        return Optional.ofNullable(newestCompatible).map(VersionCandidate::versionNumber);
    }
    ...
    private static String currentVersion() {
        return FabricLoader.getInstance()
            .getModContainer(EnderLinkMod.MOD_ID)
            .map(container -> container.getMetadata().getVersion().getFriendlyString())
            .orElse(EnderLinkMod.modVersion());
    }
    ...
    private static boolean isNewerVersion(String candidate, String current) {
        try {
            ...
            EnderLinkMod.LOGGER.debug("{} Could not compare versions. candidate='{}', current='{}'.",
                EnderLinkMod.logPrefix(),
                candidate,
                current,
                exception
